...
public class QuestionLoader {
    ...
    private static final int DEFAULT_SET_TIME_LIMIT = 30;
        ...
        try {
            ...
            for (Path filePath : jsonFiles) {
                ...
                try{
                    ...
                    // maintenant on verifie que le quiz est valide
                    if (isValidQuizz(questions)){
                        String quizzName = filename.replace(".json", "");
                        // on ajoute cela dans le retour
                        quizzes.put(quizzName, questions);
                    }else {
                        System.err.println("Fichier invalide :" + filename + "(quizz ignré)");
                    }
                }catch(IOException e){
